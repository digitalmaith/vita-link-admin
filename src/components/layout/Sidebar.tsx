  ...
  return (
    ...
    >
      ...
      <div className="px-2 pb-4 border-t border-neutral-200 dark:border-neutral-800">
        {BOTTOM_ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-muted-foreground hover:text-foreground hover:bg-neutral-100 dark:hover:bg-neutral-800/50"
            >
              <Icon className="w-4 h-4" />
        ...
        {/* User */}
        <div className="flex items-center gap-3 px-3 py-2.5 mt-2 rounded-xl bg-neutral-100 dark:bg-neutral-800/50">
      ...
      <button
        onClick={toggle}
        className={cn(
          "absolute -right-3 top-[88px] w-6 h-6 rounded-full border bg-white dark:bg-neutral-900 shadow-sm flex items-center justify-center"
        )}
      >
        {isCollapsed ? (
