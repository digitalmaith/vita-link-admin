...
// 1️⃣ Juste après les autres useRef en haut du composant
const clusterRef = useRef<Supercluster | null>(null);

// 2️⃣ Dans le useEffect des markers, remplacer tout le contenu par :
useEffect(() => {
  ...
  // ✅ Initialiser le cluster avec les structures filtrées
  ...
  const updateMarkers = () => {
   ...
   const zoom = Math.floor(mapRef.current.getZoom());
    ...
    // ✅ Guard sur bounds qui peut être null
    if (!bounds) return;
    ...
    clusters.forEach((cluster) => {
      ...
      if (isCluster) {
        ...
        // 🟢 Marker individuel (code existant)
    ...
    if (!mapBounds.isEmpty() && markersRef.current.length > 0) {
  ...
  // ✅ Mettre à jour les markers à chaque mouvement de la carte
  mapRef.current.on("move", updateMarkers);
  mapRef.current.on("zoom", updateMarkers);
  updateMarkers(); // premier rendu
